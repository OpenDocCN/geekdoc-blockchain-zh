© 作者，独家许可给 Springer Nature Switzerland AG 2022Y。Maleh 等人（编辑）区块链技术在物联网技术、通信和计算方面的进展[`doi.org/10.1007/978-3-030-93646-4_2`](https://doi.org/10.1007/978-3-030-93646-4_2)
...
+   提供对在实施区块链的 CPS 系统中应用的应用进行详细和深入的分析。
...
如果共识原则由少数指定的参与者而不是公共区块链验证，则区块链被认为是私有的。交易参与和验证任务的能力都由组织决定。存在着一个拥有预定数量节点的私有网络，比如文献中描述的位置区块链。私有区块链不需要基于密码学的技术。私有区块链上没有矿工、没有工作量证明，也没有货币奖励。这些是公共数据存储和传输系统（公钥基础设施）与私有数据存储和传输系统之间的主要区别。因此，任何形式的区块链都是一种低成本、去中心化和完全安全的存储和数据传输机制。
...
区块链的好处包括以下几点：通过共享账簿消除交易的需求，透明度和不可变性，以及交易无法被删除或改变，以及由于其完整性、一致性、日期和广泛可用性而获得的区块链数据的高质量。使用共同的交易注册表减少了由于任何故障而导致的数据丢失或不可用性的可能性。有各种由区块链管理的新概念，它们将带来重大变革。区块链是一个庞大的数据库。请看一些最常见的特征；但请记住，它们可能会根据预期的用途而改变：

一个分散的系统

区块链与传统的数字平台不同，它是一个分散的系统，每个成员都维护着一个庞大记录的副本。为了避免伪造和其他威胁，没有中央服务器，只有一个协作管理系统。由于这种去中心化，成本应该会降低。

一个透明的系统

该系统也完全开放：任何有互联网访问权限的人都可以随时查看注册表，因此也可以查看交易历史记录（或由所有网络成员查看）。因此，通过区块链，可以保证完全的资产或产品跟踪。虽然一个人使用化名，但他的行为是完全被追踪的。

一个可靠的系统

区块链无法被篡改或以任何方式更改。一旦信息记录在区块中，就无法在放置后更改或删除。有了这种新技术，电子文档可能与纸张一样有价值作为证明文件。由于副本正在增加，分散的方法也提供了一些防止盗版的保护。

一个自动化的系统

区块链提供完全的独立性，直到无缺陷的监控点，无需中间人。交易是使用计算机程序进行的。自执行的“智能合约”将可用。

一个高效的系统

所有区块链的优势汇集在一起，以保证最大的经济效益：由于消除中间人和自动化以及降低错误率和诉讼，节约时间和降低成本。当缺乏信任经常被视为增长的重大障碍时，这些资产显然是具有吸引力的。这些优势也存在缺点。区块链革命必须克服几个技术、组织和社会障碍才能取得成功。

## 3 区块链（BC）用于网络物理系统（CPS）应用

当前世界上关于区块链技术的讨论很多。根据 Gupta [11] 的说法，区块链是一个具有安全功能的信息记录系统，使得无法进行黑客攻击或在系统中作弊。Abadi 和 Brunnermeier [12] 指出，区块链系统是一个分散记录的分类帐系统，通过将记录分布在所有区块链网络中来实现。区块链系统中的交易分发给所有系统参与者，使作弊或窃取变得困难。他们进一步指出，技术的正确性、分散性和成本效益使其成为一个出色的记录保存系统。他们将该系统与传统的集中式系统进行比较，突显了区块链对记录保管行业所带来的巨大革命。区块链的一个重要特征是允许记录保管者在分类帐的历史记录中倒带和撤销虚假报告的算法。除了金融行业外，该系统在采购、互联网应用等需要高度透明度的其他行业中也非常有用。

企业成功的关键在于改善与其利益相关者的互动。区块链技术通过提供分布式账本来实现这一点。该策略允许企业利用共享的交易数据库。该技术应用了专注于交易认证和授权的加密机制。

电子商务是区块链进步极大地有益于该行业持续和整体增长的主要行业之一。区块链创新准备改变互联网商业行业。它提供了无与伦比的安全性、透明度和成本效益的组合。希望扩大企业的企业家们应该接受这一发展，并重新定义他们的工作方式。区块链技术通过提供新的有效支付、智能合约、足够的交易执行和智能合约，积极地打破了金融和电子商务领域的现状。

区块链技术为全球范围内的组织解锁了新的可能性，使其能够更加灵活、高效，并提供具有吸引力的价格、安全性和安全模型。区块链是一种使人类或实体与外部实体之间进行数字交易的技术。区块链是一个分布式数据库，存储了关于一切事物的信息。它用于存储大量关于用户、连接它们的网络以及任何其他连接设备的信息。每个区块链都被称为“钱包”或“区块链”，因为它的区块链。这些钱包存储了网络记录的信息。这些信息被称为“交易”。区块链为去中心化应用程序开发人员提供了一个框架，允许用户使用加密技术直接通信并保护其资产。它将使用户能够安全地存储商品和服务的数字签名。他们还可以互相发送消息。其中一些消息可以用于进行支付，但这仍然不止是一个中介服务。它是应用层级，应用程序可以在该层执行操作。该应用程序可以是具有区块链后端的任何操作系统或应用程序程序[13]。

人们认为区块链可以在多个层面上创建控制交易和数据流的新方式，而无需具有交换基础设施和数据中心。它允许开发人员向其添加功能作为一种高效、值得信赖和高效的解决方案。了解为什么以及如何工作区块链是有趣的。区块链的概念与传统银行不同，因为它们是由人们创建的，并且旨在通过自己进行交易和管理资产以避免风险并赚取利润。区块链是一个平台，有助于创建一个中央资源，该资源将自动充当账本，其中这些资源和记录可以验证以验证信息。

区块链无疑是主要创新。区块链在行业应用方面取得了巨大增长。它不仅促成了像 Airbnb 和 Dropbox 等公司的转型，还影响了许多其他公司。企业经营方式的这种改变将有助于推动增长并提高业务效率。它还将帮助政府提高就业率，提供更优质的服务。对企业而言，区块链技术提供了一种经济高效的替代现金支付方式。区块链技术使支付系统得以运行。它也是几种应用程序的有用功能，包括数字身份、电子商务、保险、物业管理、电子支付和众筹。该技术的一些用途如下：促进数据和信息流动；验证网络上的每笔交易；在不到 2 秒的时间内实现传统金融机构和企业无法实现的事情；以及公司和行业创造新产品和服务的速度。

区块链技术的核心是共识，这就是区块链的运作方式。区块链在多个行业对企业和行业产生了重大影响。例如，美国最大的 IT 公司之一 Accenture 表示，他们正在探索利用区块链构建基于区块链的应用程序。与任何技术一样，区块链有多种用途——例如，通过自动化合同系统或大型复杂企业的记录系统实现自动化、低延迟的竞标，或使某些形式的交易更安全高效。

本章主要关注于新兴区块链应用于网络物理系统，即医疗记录、交通运输、电子商务、金融和网络安全。表 2 列出了研究涉及的各种系统及其相应的应用领域。表 2

CPS 应用领域

| 系统 | 应用 | 好处 |
| --- | --- | --- |
| 交通运输 | 汽车电子、铁路系统、道路网络、航空和空域管理 | 简化复杂流程和设备合规管理、简化支付程序、流程可追溯、逆向物流 |
| 电子商务 | 监控和跟踪供应链以确保市场的公开性，重新设计支付系统，安全的电子商务平台，真实产品的产品见证 | 备选支付方式、订单处理、增强支付安全、更快的交易速度 |
| 医疗保健 | 汽车电子、铁路系统、道路网络、航空和空域管理 | 医疗数据管理、临床试验优化、药品追溯和防伪 |
| 财务 | 防止欺诈、金融包容性、防范洗钱、贸易金融、智能资产和智能合约 | 银行服务的 Uber 化促进资金转移更安全和高效的交易 |
| 网络安全 | 无密钥签名基础设施、用户匿名性、在网络物理系统中验证交易、数据认证 | 永久数据安全区块链上的去中心化可替代认证机构高级身份验证 |
...
患者如果将来与医疗提供者的数据分离，如果他们使用的电子健康记录（EHRs）不打算跟踪多个医疗机构和患者的记录，那么他们可能会轻易失去对过去数据的访问权。许多学者考虑使用区块链技术来维护 EHR，因为找到一种创新的方法来管理 EHR 对于鼓励患者访问他们的当前和历史健康数据至关重要。一个名为“MedRec”的原型利用了处理身份验证、保密性、完整性和简单数据交换的特定优势，一切都包含在一个包中。他们为患者提供了一个不可变的、完整的历史记录，以及通过去中心化记录管理系统从不同的医生或治疗设施获得他们的医疗信息的简单访问[37]。“MedRec”不保留任何关于您医疗护理的记录，也不需要进行调整。它会提醒负责记录位置的患者，并在区块链上记录一个标记。该标记作为您购买文件的副本的保证。同样，它通过将关心的负担转移到他们身上，为患者提供了对他们健康的更多控制。对于不希望其信息被处理的患者，管理员协会会担任患者代理的功能。今天，用户的患者条目设计复杂，需要更多的劳动，并且根据基金会有不同的用户界面。MedRec 架构中包含一个界面，以帮助在组织之间传输医疗记录时保持连接（[`medrec.media.mit.edu/`](https://medrec.media.mit.edu/)）。随着 EHR 的实施，医疗数据共享通常面临重大限制，如受控数据的丢失和可靠性和验证以及医疗信息的安全备份。由 Xia 等人开发的医疗数据共享 MeDShare[38]是一个安全的系统，确保了不受信任的各方之间的医疗数据传输。MeDShare 可以被云服务提供商、医院和健康研究用来共享医疗数据和管理电子健康记录。良好治理组织（GROs）的特征包括更大的数据来源、个性化的审计控制以及对隐私和安全的最小风险。

通常，电子健康记录（EHRs）包括高度机密的患者数据，必须在医生、放射科医生、护士、药剂师、研究人员和其他医疗行业人员之间共享，以提供准确的诊断和治疗。只要这些极度敏感的患者信息在许多组织之间存储、传输和传播，就会对患者的健康和患者医疗记录的保存构成重大危险。这些危害的发生可能比慢性疾病患者（如癌症或艾滋病患者）的历史和后续治疗、随访和康复过程更大。为了提供成功的治疗，保持患者的当前医疗史至关重要。Dubovitskaya 等人开发了基于区块链的架构，用于组织、存储和共享癌症患者的电子医疗信息，以规避这些限制[39]。为了访问、管理和存储加密的患者数据，他们转向了许可的区块链技术。这些框架可以用于将区块链技术应用于医疗活动，如访问和控制患者数据的隐私和安全性。

另一个历史性例子是爱沙尼亚的医疗记录区块链计划。今年，当它计划保留数百万份私人医疗记录并使其便于医疗专业人员和保险公司获取时，爱沙尼亚再次回到核心技术的前沿[40]。全球医疗领域区块链技术的日益普及可能对希望其医疗信息尽可能准确和不被篡改的患者来说是个好兆头。任何试图访问或进行调整的行为都可以轻松地在区块链上被识别和跟踪。这不仅有助于维护患者数据的完整性，而且还可以识别任何非法活动，如区块链欺诈或记录伪造。现在，一旦医疗保健记录被标准化，分享和评估已批准的医疗护理记录将变得更加容易。大多数患者的提供者在约定时间之前已经看过它了。考虑处方交互作用、过敏反应和药理学解决方案的患者管理算法使得可以快速地在所有区块链记录中处理所有这些事情。因此，区块链创新将受益于医疗记录管理[41]、临床信息（临床数据）更快的验证、增强的安全性和更好的护理组织。

临床研究中的区块链

临床研究中的挑战涉及患者保密性和安全性、医疗数据的隐私和完整性、记录保留以及临床试验招募等方面。区块链，互联网的下一代，有潜力解决这些问题。医疗专家正在利用区块链技术尝试找到这些问题的解决方案。区块链、人工智能（AI）和机器学习很快将接管医疗行业。Nugent 等人描述的以太坊许可协议[42]在区块链上提供了智能合约功能，这在诊所与数据管理系统中被采用。该研究的主要目标是找到解决患者招募的艰巨挑战的方案。由于研究结果，建议在临床试验中使用以太坊智能合约来实现数据管理系统的透明度，以使交易更加高效。因此，目前区块链技术在临床研究中的一个应用是患者招募。Benchoufi 等人[43]设计了一个程序，允许收集患者的知情同意，该程序与协议更改相关联，以安全、不可篡改和公开验证的方式存储和跟踪同意，并实时交换此信息。药品供应链的管理是医药业中区块链的主要用途。在许多领域，供应管理至关重要，但在医疗行业由于行业的日益复杂性而更为重要。医疗供应链的任何疏漏都直接影响患者的健康[44]。由于涉及众多移动元素和参与者，区块链极不安全，并存在许多安全漏洞可供黑客利用。由于更大的数据透明性和产品追溯性的改善，在某些情况下，区块链提供了一个安全的平台来消除这个问题并防止欺诈。操纵区块链很困难，因为区块链中的字符串无法从智能合约中确认或修改。

医药行业中的区块链

医疗保健交付最快增长和最重要的领域是制药和生物技术。这个行业帮助将新型、潜在挽救生命的药品推向市场。公众供应的医疗用品和药品通过更安全、更有效地供应而受益。因此，制药行业通过评估和处理安全药物来加速患者康复[45]。许多时候，制药公司面临着试图破坏生产或在错误的时间点混入劣质产品的伪造者，构成了重大危险。因此，伪造药物的制造和分发已成为全球重大健康危险之一，特别是在贫困国家。区块链可能是在研发过程中评估、监测和保证新药生产过程的理想解决方案。为了打击假药的制造，超级账本最近宣布了一个利用区块链技术的假药计划。

因此，在发展中国家，有迫切的需求利用数字技术跟踪、分析和确保新药的生成和分发过程的全过程。数字药物控制系统（DDCS）是解决假药问题的潜在长期答案。赛诺菲、辉瑞和安进已经启动了一个合作试点计划，利用基于区块链的 DDCS 检查和评估新药。基于区块链的策略可能有助于跟踪药品制造和位置，同时提高可追溯性。这种技术还将保护药品供应链，并验证交付给客户或最终用户的药品质量[46]。

区块链技术使得能够实时跟踪多个地点的医疗开支成本。它提供了完整的医疗费用记录。它可以帮助他们获得更便宜的健康保险或保险政策。它降低了每台服务器的成本，使医疗更加负担得起[47]。这是一项创新，使人们能够用数字资源交换他们的物理文件。区块链技术允许创建单向通信的数字证书，并通过区块链记录数据以检索数据。外科医生可以创建一个可以处理扫描并提供数字证书的软件程序。它代表了一种跟踪和管理健康数据、医疗服务和产品的新方式。它也可以应用于数字医疗行业。

区块链是一个系统，在其中个人将以不可变的格式存储信息。在需要时，他们将访问数据并执行各种操作，如转移数据。它还包括一个合同，个人同意提供数据，如果违反协议，个人将无法访问数据。个人还可以创建一个服务，其中数据可以被收集、记录和共享。区块链是一种分布式分类账技术，它实现了即时、去中心化的记录保管，允许组织、患者或患者的组织根据患者的权限在区块链上创建可信账户，安全高效。它提供了从患者历史记录和其他组织、医生、患者和医疗专业人员那里控制对机密信息的访问的能力，通过区块链应用程序，包括在线访问的那些。这些应用可以用于医疗保健和医疗行业的其他专业领域，允许访问更重要的数据。它减少了欺诈、篡改利润和交易数据落入黑客、身份窃贼或恶意实体手中的可能性。

区块链还在身份验证方面具有益处，通过增加安全性、透明度和责任性。区块链使得消费者更容易获取所需信息，而无需任何中心化系统。它通过增加安全性帮助支付、医疗记录和医疗保健记录[48]。区块链也是一种确保责任和保密性的方法。因此，它与医疗保健行业无关。然而，它是一个旨在通过提供更好的服务和透明度为明亮的医疗器械和医疗记录开发区块链技术的倡议。医疗记录和智能医疗设备的区块链技术之间没有关联。同样的技术可以提供替代传统医疗记录和电子医疗记录的方法。技术改革了医疗保健行业，并改变了它的医疗方式，并且正在帮助人们。区块链将受到全球关注，并帮助人们节省金融利益。他们可以节省医疗费用和更好的健康，对生活质量产生重大影响。

这些好处和节省将用于人们的健康，可以被利用。它会让人们节省金钱，节省金融利益。他们可以在健康和更好的生活质量上节省金钱。这些好处和节省将用于人们的健康，可以被利用。区块链技术将显著影响人们的生活，提高生活质量，使他们更加光明和快乐。它也在为人们提供健康。作为应用于医疗保健的术语，“区块链”相当广泛。在这种情况下，区块链可以被理解为用于在集中式数据库上创建和管理数据的分散式协议。虽然区块链本身不允许太多有关病人记录的信息，但一些银行可以提供数据和服务。医疗行业正在逐渐学会利用区块链来帮助人们。到目前为止，很难说这是否是正确的技术。

一种允许整个病人记录并因此为病人提供支付治疗费用的区块链是未来的趋势——不仅仅是财务记录，还包括病人办公室里的所有东西。区块链被用作医疗电子商务的延伸——这可能是改善医疗的新途径。有了区块链，通过网上订购药物可能会更便宜，但同样可靠。病历和药物等健康记录也可能更容易存储。

区块链的进步对于医疗保健行业日益重要，面临着巨额医疗费用、全球化和数字鸿沟的挑战。区块链是一种简单的技术，可以被用于善良和恶劣的目的。在医疗保健行业中，区块链的进步是一个健康产业的趋势，技术被采用以造福患者。由于区块链是一种分散式技术，它允许任何人参与市场。这是一种所有种类交易都以电子方式处理的技术。它也是一种交易透明的技术，他们不必提供任何交易历史的证明。已经有越来越多的医疗机构提供基于区块链的解决方案。
...
推进区块链是为金融和银行用户提供更快速、安全和可用的服务。区块链是转移资金的最可扩展、防篡改和安全的方式。它已经彻底改变了我们的资金在云端的保管方式，并且是数字经济中最具颠覆性的技术。最新的行业正在将这个未来应用到现实世界中。作为区块链技术的一部分，它提供了一个便于账户间进行交易的框架。因此，在过程的某一点上，金融机构的个人或实体可以充当银行和客户之间的中介。作为一家银行，在任何人完成的交易时，它可以同时充当交易经纪人。这样，个人、实体充当交易者，账户资金在一两笔交易中释放，否则会延迟。作为一家希望开发新技术的机构，它致力于成为区块链平台提供商。这是一个很好的商业策略。

这项技术可以促进科技公司。它为银行公司提供了创建区块链并提供透明度和记录的框架。随着这些变化，银行需要采用比特币。区块链在银行和金融行业的发展意义重大。此外，现在每个人都有技术来构建完全基于区块链运行的智能合约。银行现在可以向客户提供无需中央银行即可进行的金融产品。因此，这个过程正在加速，这个过程的变化速度每天都在提高。

更重要的是，一个相当大的行业已经开始研究区块链可以成为什么以及如何在各种行业中实施它，其中区块链本身无法用于此目的。例如，银行业，这个行业存在着一个巨大的问题，正在研究智能合约来管理信贷和债务，以给客户更好的银行体验。信用和借记都存储在一个特定的虚拟账户中。

区块链在银行和金融行业的进步有许多原因，包括高交易量、网络的有效利用、快速的支付结算速度和低成本。对于银行业，有几个优点，如账户详细信息的单一分类帐提供透明度和便捷访问、自动结算有助于快速且成本更低地进行大额交易、金融服务的快速支付规模使交易可能比以前更快，以及金融服务业的快速增长使得新理念更容易推进。每个注册表都是存储在名为支付详细信息的分布式分类帐中的加密存档，它也是一个数据存储系统。这两个信息系统是互补的。它们相互分开的事实就是它们互补的原因。然而，有人可能更喜欢使用一个而不是另一个。这与熟悉的账户详细信息分类帐的概念相比是一个不同的现象。

它们包含账户详细信息的副本，但这两个副本存在于具有各自状态的不同机器上。由于它们不同步，要知道它们如何同步并不容易。有人可能更喜欢在银行和金融行业使用支付信息分类帐，以降低交易速度并降低交易成本。银行和金融公司之间的差异是显而易见的。这是一种分布式分类帐技术，可以实现即时、去中心化的记录保存，允许一个组织、一家银行和一名医生。这些应用程序可以在金融行业中使用。

区块链是一个在线分布式账本，可以记录所有发生的事情。区块链账本，或者分布式账本，就像是一种数字货币。与使用实体货币不同，人们使用计算机创建虚拟货币来支付商品和服务。一个区块就是一个文件；一个人可以在区块链上写入他们的文件，并在以后阅读。区块链是一个文件记录，包括所有比特币网络记录、个人比特币钱包以及他们对其进行的任何更改，如交易或铸币。区块链是一个平台，用于管理所有金融信息的分布式、透明、去中心化和安全计算平台。它为管理数字资产和交易提供了一个对等的平台。它是一个安全的机制，用于记录交易，并充当管理数字资产的集中信任。

这个行业涉及大量的货币，一不小心就会有成千上万的交易在全球发生。区块链改进了金融操作的方式；大多数个人需要现金，但不再完整。区块链引入了更多的加密货币技术，但最为人所知的是比特币 [54]。组织和公司可以尽可能地投资；在传统交易中，没有限制。传统交易存在限制，不允许用户或组织在一次交易中拥有一定数量的资金。然而，区块链已经改变了这个行业，他们可以尽可能多地进行交易，没有任何限制。

对等全球金融交易成为可能。传统交易存在许多限制，其中一个涉及边界问题。通过第三方进行的交易在其来源国家的边界上有限制。例如，用户无法在英国完成对法国或西班牙银行账户的交易 [55]。传统交易系统仅在国内运作，在进展的情况下，仅适用于邻国。区块链已经实现了直接从发送方到接收方的过渡，无需第三方。这使得交易过程简单快速，缩短了等待验证的时间。它不需要任何第三方来验证流程，因此交易保密性。

区块链可用于保护市场通信免受不同客户、策略持有者和保险公司的干扰。它对于交换、购买和记录保险单非常有用。传统的完整交易通常是昂贵的；发送方必须支付发送费，而接收方则要支付提款费。区块链将金融行业提升到了另一个水平。使用区块链进行交易需要较少的交易费用，而且在某种程度上，完成交易时甚至没有费用。它增加了加密货币的采用，以便竞争交易，而不是使用不可靠的传统方式。

股票交易是金融行业的另一个进步，这是由于区块链技术的影响。不同的国家有不同的货币，但这在股票交易时并不是问题。卖方可能以较高的价格将货币卖给买方，但由于缺乏货币标准化，买方最终以较低的价格卖出。区块链具有透明性，货币在全球范围内保持一致。比特币货币的价格在所有国家都保持标准；如果它上涨，那么下跌时也会有相应的影响。它使交易永久化，并减少了关于传统交易的网络活动。

区块链技术对所有行业都至关重要，在激发生产力方面发挥着至关重要的作用。通过使用加密机制，此技术在安全性方面非常高。只有拥有完美加密密钥的人才能解密信息。作为完成交易的方式，区块链技术没有任何限制。只要有互联网连接，它就可以随时使用。事实上，由于区块链技术的出现，金融业发生了变革，使你可以节省所拥有的资金。区块链确实使金融行业成功，因为它具有无限数量的交易，且交易费用较低。它还允许更多的客户购买全球范围内的产品，因为货币标准化。

世界上关于区块链技术的讨论很多。根据[11]，区块链是一个带有安全特性的信息记录系统，使其无法被黑客攻击或在系统中作弊。阿巴迪等人[12]指出了一个分布式账本系统，通过将记录分布到所有区块链网络中来实现去中心化。区块链系统中的交易分布到所有系统参与者，使作弊或窃取变得困难。他们进一步指出，技术的正确性、去中心化和成本效益使其成为一种出色的记录系统。他们对该系统与传统集中式系统的比较突显了区块链对记录行业带来的巨大革命。区块链的一个重要特性是允许记录员回溯和撤销账本历史记录中的错误报告的算法。除了金融业，该系统在采购、互联网应用等其他需要高度透明度的行业中也非常有用。我将重点关注金融业，探讨该技术在该行业中的进展。

防欺诈

区块链在金融行业中极大地减少了欺诈行为。处理资金和资产交易的金融机构极易受到欺诈或犯罪带来的损失影响。金融部门过去依赖于集中式系统进行记录。黑客和犯罪分子对这种设计非常熟悉，他们可以轻易操纵它，因为一旦进入这样的系统，他们就能拥有做任何事情的绝对权力。区块链是一种安全、不可腐败的技术，依赖于一个硬性的去中心化网络，使攻击者无法操纵或渗透。每一笔交易都以密码学机制的形式记录和存储。这种机制几乎不可能被破坏，即使被破坏，也有简单的方法追踪攻击者。困难在于连接所有区块，以便一旦发现一个漏洞，它们都会检测并显示变化。连接的区块还缩短了追踪漏洞的时间，减少了攻击者在系统中进行任何非法业务的时间。

金融包容

当前的银行监管和限制极大地阻碍了许多人使用银行，他们不得不寻找替代解决方案。金融包容性是每个人都能利用正式金融系统进行经济增长和发展的能力和机会。与区块链相关的低成本使初创公司有机会与中央银行竞争。初创公司依赖于具有数字身份识别和移动设备的替代方案来访问金融服务。这种无忧的系统为愿意为小银行家提供服务的创新者提供了竞争机会，从而实现了金融包容性。

防止洗钱

随着大多数发达和发展中经济体实施反洗钱法规，了解客户政策使客户在银行机构注册成为一件相当昂贵的事情。据估计，金融机构在将客户录入其记录时花费的费用介于 6,000 万美元到 5 亿美元之间。他们被要求进行背景调查，或者通常被称为客户尽职调查。该过程旨在减少或消除全球洗钱，并遏制恐怖分子和毒品组织等犯罪组织。一个银行或机构对客户进行的尽职调查将客户信息访问到区块链系统中的其他金融机构。工作量大大减少，并且同一行业中不会重复努力。区块链系统带来的这种优势极大地激励了金融部门的业务领导者获取并加入区块链系统，以降低其运营成本，优化其组织的利润 [11]。
...
下一组研究包括致力于解决多个设备之间建立可信交互的问题。与这些设备操作的一些数据的完整性有关的具体任务可能有所不同。
...
算法将数字水印组件一致而独立地嵌入感知数据中，因为它们不依赖于这些感知数据的值或它们的某些特征。虽然传统的数字水印方法和算法以及无线传感器网络和物联网的问题可以基于受保护的数据创建数字水印，但这个概念是非常广泛的。

在最简单的例子中，数字水印组件仅根据传感器数据元素的值创建。[86]中提供的嵌入方法就是一个例子。该方法根据先前的传感器值生成插入到下一个传感器值中的数字水印位。

数字水印组件的单独嵌入提供了各种优势；尽管如此，这种技术在时序方面存在挑战。想象一下如果一条消息的数据顺序混乱会发生什么。即使通信通道上没有主动入侵者，数字水印提取也会受到阻碍。王等人[87]提出了对这一困境的解决方案。传感器数据根据密钥分成变长组，如作者在其论文中提出的那样。为相邻组合生成数字水印链，并植入。一系列数字水印用于验证传感器数据。第二个数字水印链提供分隔符和数据同步，编码组分隔符。

在更复杂的情景下，创建包含传感器数量值及其某些特性的数字水印是可能的。Ferdowsi 等人[88]提出了一种用于动态认证 IoT 信号以检测网络攻击的数字水印算法。IoT 设备（IoTDs）可以从其产生的信号中提取一组随机特性，并使用建议的水印技术将这些特性动态地水印到信号中。该方法允许 IoTDs 在鉴别由 IoT 网关收集的信号时进行鉴别。Hameed 等人[89]还讨论了如何使用所获得数据的几个方面来创建数字水印，例如数据长度、出现频率和捕获时间。Nguyen 等人[90]根据 CSMA/CA 协议冲突创建数字水印，以防止传感器节点克隆攻击。此外，传感器数据的呈现方式也是研究的一个特点。它们结合在一起，形成类似于数字图像的矩阵。总的来说，这样的系统使得在处理具有传感器数据的数字图片时采用已被证明成功的方法成为可能。

在提到的研究中，使用表示二进制序列的数字水印，所有算法都有效。对模拟传输（特别是调制信号）进行水印处理以解决信号源认证也是一个研究领域。在那些作品中发现的答案与数字水印中发现的答案理想地是可比的。区别仅在信号的表示方式以及因此信号的处理方式上。

系统中与 NB-IoT（窄带物联网）标准相匹配的发件人认证侧重于研究[91]。研究中使用了射频水印的概念。与将来嵌入二进制序列不同，水印首先被创建为数字水印，然后转换为调制信号。建议技术的主要优点是更可靠，因为可用信号和水印信号不会互相冲突。

在某些情况下，水印可以用于防止某些类型的攻击。Rubio-Hernan 等人[92]提出了一种适应性控制理论技术，用于检测网络化控制工业系统上的网络复制尝试。这指的是入侵者通过复制先前拦截的数据序列来篡改系统控制。这项工作的根本贡献不是嵌入技术，而是使用此算法来防范入侵者的方法，该算法来源于先前的出版物。

黄等人[93]提出了一种在“硬”实时工业控制系统中传递信号时嵌入可逆空气符号的技术。作者选择船舶控制系统作为最重要的应用领域。在嵌入开始之前，必须通过安全通信渠道提前交付秘钥。这里描述的方法可以检测到旨在导致信号延迟和失真的攻击。

最后，根据前述部分的概述，已经开始实验将区块链与数字水印技术整合。区块链和数字水印解决了网络物理系统中的不同安全问题。它们的联合使用可能比单独使用这些方法提供更高水平的安全性。这个概念以前在先前的作品中已经探讨过，但主要是在一个方面，即数字版权管理的问题[79, 91]。这些技术在其他领域的协同实施[7, 94, 95]是一个有前途的研究领域，其发展将有益于网络安全。表 5 总结了网络安全中的许多区块链用途。表 5

区块链在网络安全中的应用

| 应用领域 | 应用 | 贡献 |
| --- | --- | --- |
| 受量子启发的区块链 [96] | 物联网智能城市中的智能边缘设施 | 抵御来自数字和量子计算机的潜在攻击 |
| 基于轻量级区块链的网络安全（LBC）[97] | 物联网环境 | 解决计算需求和带宽消耗的问题 |
| 区块链强化的合作认证 [97] | 车辆边缘计算 | 保护和保留数据的隐私和机密性，同时确保互相认证（例如，回复攻击） |
| BloCyNfo-Share [98] | 网络安全信息交换（CYBEX） | 描述如何与其他组织共享私人信息或向其他组织提供私人信息的访问 |
| BBDS [99] | 云环境中的电子医疗记录 | 旨在将医疗数据传播到受保护机构云之外 |
| Ancile [100] | 电子医疗记录 | 保护患者敏感信息的隐私 |
| 安全且去中心化的共享[101] | 跨领域的图像共享。个人健康数据可以使用基于 Hyperledger Fabric 的树状系统批量处理 | 允许各方在不依赖单一权威的情况下达成协议 |
| 区块链[23] | 相互连接的智能车辆 | 解决智能车面临的安全和隐私威胁，如位置跟踪和远程车辆劫持 |

## 区块链的局限性和未来发展方向

尽管区块链技术承诺着许多安全性问题，但仍然存在许多安全问题，正如报道的许多数据泄露和欺诈案例所示。对于一个其关键标榜的属性是可靠性和不可侵犯性的系统来说，这是一个自相矛盾的陷阱。另一方面，计算机攻击会对与其他系统的所有类型的交互产生影响——主要是市场，这些市场容易受到中心化系统的经典缺陷的影响，如银行[102]。

总的来说，关于区块链的速度、吞吐量、保密性、可扩展性和互操作性问题已经得到证实并且有充分的记录。挖矿问题是比特币共识所基于的工作证明机制的核心，也是备受争议的问题。在理论层面上，严格定义能够保护免受恶意验证节点影响的条件是很重要的。恶意实体持有的计算能力达到 51%被视为参考水平。然而，这个数值在研究界存在争议。节点的数量和分布也是一个敏感的问题。经济问题也开始引起研究者的关注[10]。除了货币和金融方面的问题（货币之间的竞争、货币系统），还涉及到挖矿经济学问题，比如矿工合作激励或者挖矿能力的演变。讨论不仅仅局限于比特币，因为许多问题取决于选定的替代中心化决策系统的共识模型。无论情况如何，分布式账本的技术环境都将继续显著发展。

在区块链上完成交易所需的时间是一个主要的缺点。由于比特币网络的广大，这个过程可能需要很多小时才能完成。使用像 Hyperledger Sawtooth 这样的区块链有助于减少这种延迟，因为它允许使用 PoET（Proof of Elapsed Time）共识过程，这是反应时间最快、最不耗资源的过程之一，使其更适合我们当前的环境，包括网络化的对象数据和服务[103]。其次，并不是每个人都应该能够访问区块链数据。这个挑战可以通过利用私有区块链来解决，私有区块链可以管理区块链访问权限和交易执行权。其次，区块链的共识算法，特别是 PoW（工作量证明），在计算方面非常耗能。

此外，决定是否可以向区块链添加新块的冗余数据和冗余计算需要大量能源。最终，区块链代表了思维方式的巨大变革。换句话说，网络正在从中央集权转向去中心化。由于这个原因，客户可能难以将这项技术纳入他们现有的生态系统中并与之整合。

区块链的其他限制经常被提及，特别是透明度与保密性之间、匿名性与利益相关者识别之间的紧张关系。由于注册信息广泛传播，利益相关者可以轻松访问其中包含的明文信息。在追踪交易时，这是一个好处，但在公司保密性方面，比如银行或医疗保健方面，这是一个禁止问题。市场正在寻找可靠地掩盖信息的方法，同时参与需要披露一些信息并保护其他信息的活动[58]。

另一个限制是所需的高能量量。使用区块链需要大量耗电的验证、验证和加密程序。如果这种技术被广泛使用，可能会产生重要的负面环境外部性。

尽管区块链受到了广泛关注，但必须承认分布式账本并非万能药。区块链尚不适合快速处理大量数据，特别是视频和音频，以及在快速变化的环境中使用。区块链非常适合长期存储信息，尤其是那些不经常变化的信息。因此，该技术有望用于从银行、医疗机构以及保险和物流公司收集客户数据[104]。分布式交易注册将使专利局和地籍局受益。该技术适用于执法和税务机构记录个人数据。经纪和投资公司将从区块链作为交易登记处受益。当前技术的能力只是中间阶段。区块链的持续改进为其在新兴行业中的应用打开了前景。在其演变过程中，任何技术都必须克服变革的阻力。区块链已经度过了那个阶段，因此它将继续发展。

## 5 结论

计算和通信技术的快速进步推动了科学界和工业界对网络物理系统的兴趣。利用传感器、计算和网络能力，网络物理系统为提供各种领域的自动决策过程做出了新一代的科学和技术解决方案的贡献，从小型家庭过程的自动化到材料运输、未来工厂和关键任务产业。通过将信息技术方法与电力网、交通系统和供应链等物理系统集成，可以实现更高效、可靠和可持续的智能基础设施。

介绍了区块链在网络物理系统控制协议中的各种应用概述。虽然每个行业在使用区块链方面都有优势，但也存在挑战。然而，区块链因其在各行各业中的各种优势而广为人知并得到采用。

本章通过提供科学文献中较少记录的主题信息来为主题领域做出贡献。在工作开发过程中，清楚地表明了该主题值得进一步进行理论研究。结果分析表明，所讨论的主题每年都在增长，并且对于利用区块链为网络物理系统开发新应用而言具有重要意义。目前还为时尚早，不能确定区块链技术在网络物理系统应用的背景下是否更为合适，也不能将其与已经使用的其他技术进行比较。
